...
class VoiceAssistant:
    ...
    def __init__(self, system_prompt: str = "You are a helpful assistant."):
        ...
        self.vad_window_size = 512  # Silero VAD expects 512 samples at 16kHz (32ms)  # Increased window size (96ms)
...
async def create_call(
    ...
    global assistant
    # PATCH: Only load the model if assistant is None, otherwise update prompt/voice
    if assistant is None:
        assistant = VoiceAssistant(system_prompt=config.systemPrompt)
        if config.voice:
            assistant.current_voice = config.voice
    else:
        assistant.system_prompt = config.systemPrompt
        if config.voice:
            assistant.current_voice = config.voice
