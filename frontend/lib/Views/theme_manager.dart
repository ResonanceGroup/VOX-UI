...
class ThemeManager extends ChangeNotifier {
  ...
  void _updateBrowserThemeColor() {
    ...
    try {
      ...
      applyThemeMeta();
      // Repeat across the next paint frames; WebKit sometimes snapshots browser
      // chrome before the first DOM mutation is composited.
      html.window.requestAnimationFrame((_) => applyThemeMeta());
      html.window.requestAnimationFrame((_) {
        html.window.requestAnimationFrame((_) => applyThemeMeta());
      });
      html.document.documentElement?.style.setProperty('color-scheme', isDarkMode ? 'dark' : 'light');
      html.document.body?.style.setProperty('color-scheme', isDarkMode ? 'dark' : 'light');
